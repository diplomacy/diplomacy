...
Diplomacy package actually provides 2 clients:
:doc:`the Python client </tutorial/client>` and :doc:`the web front-end </tutorial/web>`.
...
class _AbstractGameRequest(_AbstractChannelRequest):
    ...
    Game ID, game role and phase fields are automatically filled
    by a NetworkGame object before sending request.
    """
...
class GetDaidePort(_AbstractRequest):
    ...
    :raise diplomacy.utils.exceptions.DaidePortException: if there is no DAIDE port associated
        to given game ID.
...
class ListGames(_AbstractChannelRequest):
    ...
    :param game_id: if provided, look for games with game ID either containing or contained into
        this game ID.
    :param status: if provided, look for games with this status.
    :param map_name: if provided, look for games with this map name.
    :param include_protected: (default True) tell if we must look into
        games protected by a password
    :param for_omniscience: (default False) tell if we look for games where
        request user can be at least omniscient.]
    :type game_id: str, optional
    ...
    :return:
        ...
        - Client: a list of :class:`.ComplexDataGameInfo` objects, each containing
...
class GetGamesInfo(_AbstractChannelRequest):
    ...
    :return:
        ...
        - Server: :class:`.DataGames`
        - Client: a list of :class:`.ComplexDataGameInfo` objects.
    """
    __slots__ = ['games']
    params = {
...
class ClearCenters(_AbstractGameRequest):
    ...
    :param power_name: if given, clear centers for this power.
        Otherwise, clear centers for all powers.
    :type power_name: str, optional
    :return: None
    """
...
class ClearOrders(_AbstractGameRequest):
    ...
    :param power_name: if given, clear orders for this power.
        Otherwise, clear orders for all powers.
    :type power_name: str, optional
    :return: None
    """
    __slots__ = ['power_name']
...
class ClearUnits(_AbstractGameRequest):
    ...
    :param power_name: if given, clear units for this power.
        Otherwise, clear units for all powers.
    :type power_name: str, optional
    :return: None
    """
    __slots__ = ['power_name']
...
class GetAllPossibleOrders(_AbstractGameRequest):
    ...
    Return (server and client) a :class:`.ComplexDataPossibleOrders` object
...
class QuerySchedule(_AbstractGameRequest):
    ...
    Returns (server and client) a :class:`.ComplexDataGameSchedule` object.
    """
    __slots__ = []
...
class SetDummyPowers(_AbstractGameRequest):
    ...
    :param power_names: list of power names to set dummy.
        If not provided, will be all map power names.
    :param username: if provided, only power names controlled by this user
        will be set dummy.
    :type power_names: list, optional
...
class SetGameState(_AbstractGameRequest):
    ...
    params = {
        ...
        strings.MESSAGES: parsing.DictType(int, parsing.JsonableClassType(Message),
...
class SetOrders(_AbstractGameRequest):
    ...
    :param power_name: power name. If not given, request user must be a game player, and power
        is inferred from request game role.
    :param orders: list of power orders. See :doc:`recommended order syntax </information/syntax>`,
        and associated game method :meth:`.Game.set_orders`.
...
class SetWaitFlag(_AbstractGameRequest):
    ...
    :param power_name: power name. If not given, request user must be a game player, and power
        if inferred from request game role.
...
class Synchronize(_AbstractGameRequest):
    ...
    :type timestamp: int
    :return: (server and client) a :class:`.ComplexDataGameInfo` object.
    """
    __slots__ = ['timestamp']
    params = {
...
class Vote(_AbstractGameRequest):
    ...
    :param power_name: power name who wants to vote. If not provided, request user must be a
        game player, and power name will be inferred from request game role.
    :param vote: vote to set. Either ``'yes'`` (power votes for draw), ``'no'`` (power votes
        against draw), or ``'neutral'`` (power does not want to decide).
    :type power_name: str, optional
    :type vote: str
    :return: None
    """
